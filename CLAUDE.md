...
| 列 | 内容 | 備考 |
|---|---|---|
| A | タスク名 | 空にするとカレンダー側のイベントを削除する（削除トリガーの1つ。もう1つは「キャンセル」ステータス。詳細はD列参照） |
| B | 開始日時 | `YYYY/MM/DD HH:mm` 形式（1桁の月・日・時・分も許容）。**空欄の場合は終日タスクとして登録される**（日付はトリガー実行日） |
| C | 終了日時 | 同上。空欄時は開始+1時間（終日タスクの場合は無視される） |
| D | ステータス | 未着手 / 進行中 / 完了 / キャンセル。未入力の場合はSheets→Calendar同期時に「未着手」が自動補完される。**「キャンセル」にするとCalendarイベントが削除される**（Sheet行自体は残る。Sheets側での変更・Calendar側での色/ラベル変更どちらでも削除される。Issue #44） |
...
- ステータスを**「キャンセル」にするとCalendarイベントが削除される**（Sheet行自体は残る）。Sheets側での変更・Calendar側での色/ラベル変更どちらでも削除がトリガーされる（Issue #44）
