...
**Calendar側で色/ラベルを指定せずに新規登録された場合**（`newStatus` が上記3段階いずれでも解決できないケース）：Sheet側には従来通り「未着手」がデフォルト補完されるが、それに加えてCalendarイベント自体にも `_applyStatusColorAndLabel()` で「未着手」の色・ラベルを書き戻す（Issue #42）。Sheet表示とCalendar表示の色が食い違わないようにするための対応。
